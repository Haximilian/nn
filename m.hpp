...
{
private:
    std::unique_ptr<std::array<T, R * C>> arr;
...
public:
    ...
    matrix(std::unique_ptr<std::array<T, R * C>>, T (*f)());
    ...
    std::unique_ptr<abstract_matrix<T>> operator*(const abstract_matrix<T>*) const;
...
template<typename T, size_t R, size_t C>
matrix<T, R, C>::matrix() {
    this->arr = std::unique_ptr<std::array<T, R * C>>(new std::array<T, R * C>());
}
...
template<typename T, size_t R, size_t C>
matrix<T, R, C>::matrix(std::unique_ptr<std::array<T, R * C>> arr, T(*f)()) {
...
template<typename T, size_t R, size_t C>
std::vector<T> matrix<T, R, C>::operator*(const std::vector<T> in) const {
    std::vector<T> out(R);
