...
typedef Vector<float> activation_fn(Vector<float>);
...
public:
    ...
    std::vector<Vector<T>> ForwardPropagation(Vector<T> in);

    std::vector<Matrix<T>> CalculateGradient(std::vector<Vector<T>> activations, Vector<T> actual);
    ...
    Vector<T> actual)
{
    ...
    activation_derivative[0] = Identity<T>(activations[0].Size());
